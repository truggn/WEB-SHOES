...
export default class PhieunhapsController {
  public async index({ view }: HttpContextContract) {
    ...
    return view.render('admin/nhapHang', { dataPN })
  }
  ...
  public async create({ view }: HttpContextContract) {
    ...
    return view.render('admin/danhMucNhapHang', { dataKhachHang, dataLoaiSP, dataNCC, })
  }
  ...
  public async store({ response, request, auth }: HttpContextContract) {
    ...
    const dataPN = request.only(['tongtien', 'vat', 'solo', 'trangthai', 'id_user', 'id_ncc', 'ghichu', 'tratruoc'])
    ...
    if (ncc != null) {
      ...
      const dataCTPN = request.only(['id_pn', 'tensanpham', 'soluong', 'gianhap', 'thanhtien', 'size'])
      var tongTien = 0;
